...
-- Column names match the agent's expected schema (e.g. address_stateOrRegion, facilityTypeId).
CREATE VIEW IF NOT EXISTS facilities AS
SELECT
  o.id AS pk_unique_id,
  o.id AS unique_id,
  ...
  o.lat,
  o.lon,
  (SELECT group_concat(specialty) FROM organization_specialties s WHERE s.organization_id = o.id) AS specialties,
  (SELECT group_concat(value) FROM organization_facts f WHERE f.organization_id = o.id AND f.fact_type = 'procedure') AS procedure,
  (SELECT group_concat(value) FROM organization_facts f WHERE f.organization_id = o.id AND f.fact_type = 'equipment') AS equipment,
  (SELECT group_concat(value) FROM organization_facts f WHERE f.organization_id = o.id AND f.fact_type = 'capability') AS capability
